...
model_kin = vm.KinematicBicycleConstantSpeed('config_files/mpc_bicycle_config.yaml') 
n_horizon = 30
...
current_state = np.array([.0, 3.5, 0.])
X_ref = [np.array([1.0 * (i+1) * model_kin.mpc_params['dt'], 0., 0.0]) for i in range(n_horizon)]
U_ref = [np.zeros((1, )) for i in range(n_horizon)]

X_guesss, U_guesss = mpc_test_2.solve_sqp(current_state, X_ref, U_ref, debug = True, sqp_iter=3, alpha=0.15)
...
current_state = np.array([4, -15, 0.5])
...
while j < n_total_traj - n_horizon:
    ...
    if n_sim%100 == 1:
        X_guess, U_guess = mpc_test_2.solve_sqp(current_state, X, U, debug = True, sqp_iter=8, alpha=0.2)
        input()
    else:
